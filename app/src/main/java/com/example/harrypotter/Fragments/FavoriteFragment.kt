...
class FavoriteFragment : Fragment(R.layout.fragment_favorite) {
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val charList = view.findViewById<RecyclerView>(R.id.favoriteCharacterList)

        val database = OverallDatabase.getDatabase(requireContext())

        val emptyData = view.findViewById<LinearLayout>(R.id.emptyData)

        database.favoriteDao().getFavoriteAll().observe(viewLifecycleOwner, {
            if (it.size == 0) {
                emptyData.visibility = View.VISIBLE
            } else {
                emptyData.visibility = View.GONE
            }
            val adapt = FavoriteAdapter(requireContext(), it)
